...
{
  float *nn_output_buff = Ai_Context_Ptr->nn_output_buffer;

  /* in-place operation */
  ai_array sm_array = AI_ARRAY_OBJ_INIT(
    AI_ARRAY_FORMAT_FLOAT, nn_output_buff, nn_output_buff,
    AI_NET_OUTPUT_SIZE * sizeof(float));
  nl_func_sm_channel_f32(&sm_array, &sm_array, AI_NET_OUTPUT_SIZE, AI_HANDLE_NULL);
}
