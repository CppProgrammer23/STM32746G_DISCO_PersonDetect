  ...
  for (int row = 0; row < NN_OUTPUT_CLASS_NUMBER; row++)
  {
    for (int col = 0; col < NN_OUTPUT_CLASS_NUMBER; col++)
    {
      ...
      if (value > 0)
      {
        ...
        GUI_SetBackColor(heat_map[heat_map_idx]);
        uint32_t text_color = heat_map_idx > 5 ? GUI_COLOR_BLACK : GUI_COLOR_WHITE;
        GUI_SetTextColor(text_color);
      ...
      GUI_DisplayStringAt(x_off + col * 20, y_off + row * 20, (uint8_t *)conf_value, LEFT_MODE);
 ...
 *
 * @param report ClassificationReport_Typedef structure to be dispayed
 */
static void DisplayClassificationReport(TestContext_TypeDef * TestContext_Ptr, const ClassificationReport_Typedef *report)
{
  ...
  GUI_DisplayStringAt(320, LINE(0), (uint8_t *)"precision recall f1-score support", LEFT_MODE);
  for (uint32_t target = 0; target < AI_NET_OUTPUT_SIZE; target++)
  {
    ...
    GUI_DisplayStringAt(40, LINE(2 + target), (uint8_t *)line, LEFT_MODE);
  }

  sprintf(line, "%20s %26.3f %4d", "accuracy", report->accuracy, (unsigned int)report->total_support);
  GUI_DisplayStringAt(40, LINE(3 + AI_NET_OUTPUT_SIZE), (uint8_t *)line, LEFT_MODE);
  ...
  GUI_DisplayStringAt(40, LINE(4 + AI_NET_OUTPUT_SIZE), (uint8_t *)line, LEFT_MODE);
  ...
  GUI_DisplayStringAt(40, LINE(5 + AI_NET_OUTPUT_SIZE), (uint8_t *)line, LEFT_MODE);
...
* @param Test_Context_Ptr pointer to utilities context
* @retval None
*/
static void FrameCaptureInit(TestContext_TypeDef *Test_Context_Ptr) 
{
  ...
  GUI_DrawRect(200, 10, 400, 50, GUI_COLOR_WHITE);
  GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"Capture file format", CENTER_MODE);
  
  GUI_DisplayStringAt(0, LINE(5), (uint8_t *)"Press one of the key of the joystick", CENTER_MODE);
  GUI_DisplayStringAt(0, LINE(6), (uint8_t *)"to select the format of the captured files", CENTER_MODE);
  ...
  GUI_DrawLine(pt_center.x - pt_offset, pt_center.y, pt_center.x, pt_center.y - pt_offset, GUI_COLOR_WHITE);
  GUI_DrawLine(pt_center.x, pt_center.y - pt_offset, pt_center.x + pt_offset, pt_center.y, GUI_COLOR_WHITE);
  GUI_DrawLine(pt_center.x + pt_offset, pt_center.y, pt_center.x, pt_center.y + pt_offset, GUI_COLOR_WHITE);
  GUI_DrawLine(pt_center.x, pt_center.y + pt_offset, pt_center.x - pt_offset, pt_center.y, GUI_COLOR_WHITE);
  
  const char *appli_names[] = {"RAW", "BMP"};
  
  GUI_DisplayStringAt(0, pt_center.y - pt_offset - LINE(2), (uint8_t*)appli_names[0], CENTER_MODE);//UP
 // GUI_DisplayStringAt(pt_center.x + pt_offset + 10, pt_center.y - LINE(1) / 2, (uint8_t*)appli_names[1], LEFT_MODE);//RIGHT
  GUI_DisplayStringAt(0, pt_center.y + pt_offset + LINE(1), (uint8_t*)appli_names[1], CENTER_MODE);//DOWN
 // GUI_DisplayStringAt(20, pt_center.y - LINE(1) / 2, (uint8_t*)appli_names[3], LEFT_MODE);//LEFT 
  ...
  /*else
  {
    ...
    {
      joy_state = BSP_JOY_GetState(JOY1, 0);
    }
  ...
  {
    GUI_DisplayStringAt(0, LINE(14), (uint8_t *)"Error. SD Card not detected", CENTER_MODE);
    DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
    //BSP_LED_On(LED_RED);
    while (1); 
  }
...
static void MemoryDumpInit(TestContext_TypeDef *Test_Context_Ptr)
{
  ...
  GUI_DrawRect(200, 10, 400, 50, GUI_COLOR_WHITE);
  GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"Memory dump sub-mode", CENTER_MODE);
  
  //GUI_DisplayStringAt(0, LINE(5), (uint8_t *)"Press one of the key of the joystick", CENTER_MODE);
  GUI_DisplayStringAt(0, LINE(6), (uint8_t *)"to select the frame source", CENTER_MODE);
  ...
  GUI_DisplayStringAt(pt_center.x + pt_offset + 10, pt_center.y - LINE(1) / 2, (uint8_t*)appli_names[1], LEFT_MODE);//RIGHT
  //GUI_DisplayStringAt(0, pt_center.y + pt_offset + LINE(1), (uint8_t*)appli_names[1], CENTER_MODE);//DOWN
  GUI_DisplayStringAt(200, pt_center.y - LINE(1) / 2, (uint8_t*)appli_names[2], LEFT_MODE);//LEFT 
  ...
  if(Test_Context_Ptr->DumpContext.Dump_FrameSource==SDCARD_FILE)
  {
    ...
    {
      GUI_DisplayStringAt(0, LINE(14), (uint8_t *)"Error. Directory 'dataset' doesn't exist", CENTER_MODE);
      DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
      //BSP_LED_On(LED_RED);
      while (1)
        ;
    ...
    if (nbr_dir != 0)
    {
      GUI_DisplayStringAt(0, LINE(14), (uint8_t *)"Error. Number of found directories incorrect", CENTER_MODE);
      DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
      //BSP_LED_On(LED_RED);
      while (1)
        ;
...
static void OnBoardValidInit(TestContext_TypeDef *Test_Context_Ptr)
{
  ...
  if (ret == STM32FS_ERROR_DIR_NOT_FOUND)
  {
    GUI_DisplayStringAt(0, LINE(14), (uint8_t *)"Error. Directory 'onboard_valid_dataset' doesn't exist", CENTER_MODE);
    DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
   // BSP_LED_On(LED_RED);
    while (1)
      ;
  ...
  {
    GUI_DisplayStringAt(0, LINE(14), (uint8_t *)"Error. Unable to create the missclassified file", CENTER_MODE);
    DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
    //BSP_LED_On(LED_RED);
    while (1)
      ;
  ...
  GUI_DisplayStringAt(0, LINE(1), (uint8_t *)tmp_msg, CENTER_MODE);
  ...
  { /* Class index was not found */
    ...
    GUI_DisplayStringAt(0, LINE(3), (uint8_t *)tmp_msg, CENTER_MODE);
    DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
    //BSP_LED_On(LED_RED);
    while (1)
      ;
...
static void DisplayIntroMessage(TestContext_TypeDef *Test_Context_Ptr)
{
  ...
  GUI_Clear(GUI_COLOR_BLACK);
  GUI_DrawRect(200, 10, 400, 50, GUI_COLOR_WHITE);
  GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"Onboard Validation", CENTER_MODE);

  GUI_DisplayStringAt(margin, LINE(4), (uint8_t *)"Onboard validation will look for a directory", LEFT_MODE);
  GUI_DisplayStringAt(margin, LINE(5), (uint8_t *)"named 'onboard_valid_dataset_vga or _qvga'", LEFT_MODE);
  GUI_DisplayStringAt(margin, LINE(6), (uint8_t *)"at the root of the SDCard.", LEFT_MODE);
    GUI_DisplayStringAt(margin, LINE(7), (uint8_t *)"This directory should contain one directory", LEFT_MODE);
  GUI_DisplayStringAt(margin, LINE(8), (uint8_t *)"per class containing images in BPM 16 bpp", LEFT_MODE);
  GUI_DisplayStringAt(margin, LINE(9), (uint8_t *)"format.", LEFT_MODE);

  GUI_DisplayStringAt(margin, LINE(12), (uint8_t *)"Please insert the SDCard now,", LEFT_MODE);
  GUI_DisplayStringAt(margin, LINE(13), (uint8_t *)"then press the WAKE-UP button to get started", LEFT_MODE);
...
static void Capture_PostProcess(TestContext_TypeDef *TestContext_Ptr)
{
  ...
  {      
    GUI_DisplayStringAt(0, LINE(1), (uint8_t *)TestContext_Ptr->CaptureContext.capture_session_name, LEFT_MODE);
    GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"CAPTURE READY", RIGHT_MODE);
...
static void Dump_PostProcess(TestContext_TypeDef *TestContext_Ptr)
{
  ...
  {      
    GUI_DisplayStringAt(0, LINE(1), (uint8_t *)TestContext_Ptr->DumpContext.dump_session_name, LEFT_MODE);
    GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"DUMP READY", RIGHT_MODE);
    ...
    GUI_DisplayStringAt(0, LINE(DISPLAY_TOP_N_LAST_LINE - NN_TOP_N_DISPLAY + 0), (uint8_t *)msg, CENTER_MODE);
      ...
      if(TestContext_Ptr->UartContext.uart_host_requested_dump_number == 0)
      {
        ...
        *(aTxBuffer) = cmd_status;
        ...
        Uart_Rx(TestContext_Ptr, aRxBuffer, RX_TRANSFER_SIZE);
         ...
         BSP_SD_DeInit();/*Required in order to avoid subsequent call to Camera_Init() fct to fail*/
      }
      else
      { /*Re-trigger a dump*/
...
static void Validation_PostProcess(TestContext_TypeDef *TestContext_Ptr)
{
  ...
  if(TestContext_Ptr->ValidationContext.validation_completed == 0)
  {
    ...
    if (predicted_class != TestContext_Ptr->ValidationContext.class_index)
    {
      ...
      {
        GUI_DisplayStringAt(0, LINE(14), (uint8_t *)"Error. Unable to write to missclassified file", CENTER_MODE);
        DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
        while (1)
          ;
      }
    ...
    * As the output comes from the softmax layer, the loss is the negative log of confidence */
    ...
    GUI_DisplayStringAt(40, LINE(21), (uint8_t *)tmp_msg, LEFT_MODE);
    sprintf(tmp_msg, "Average loss (categorical cross-entropy) %.4f ", TestContext_Ptr->ValidationContext.avg_loss);
    GUI_DisplayStringAt(40, LINE(22), (uint8_t *)tmp_msg, LEFT_MODE);
 ...
 * @param  Pointer to Utilities context
 */
...
void TEST_MainMenu(TestContext_TypeDef *Test_Context_Ptr)
{
  ...
  GUI_DrawRect(200, 10, 400, 50, GUI_COLOR_WHITE);
  GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"Operating mode", CENTER_MODE);

  GUI_DisplayStringAt(0, LINE(5), (uint8_t *)"Press one of the key of the joystick", CENTER_MODE);
  GUI_DisplayStringAt(0, LINE(6), (uint8_t *)"to select an operating mode", CENTER_MODE);
  ...
  GUI_DisplayStringAt(0, pt_center.y - pt_offset - LINE(2), (uint8_t*)appli_names[0], CENTER_MODE);//UP
  GUI_DisplayStringAt(pt_center.x + pt_offset + 10, pt_center.y - LINE(1) / 2, (uint8_t*)appli_names[1], LEFT_MODE);//RIGHT
  GUI_DisplayStringAt(0, pt_center.y + pt_offset + LINE(1), (uint8_t*)appli_names[2], CENTER_MODE);//DOWN
  GUI_DisplayStringAt(150, pt_center.y - LINE(1) / 2, (uint8_t*)appli_names[3], LEFT_MODE);//LEFT
...
void TEST_GetNextValidationInput(TestContext_TypeDef *TestContext_Ptr, uint8_t * DestBuffPtr)
{
  ...
  FIND_NEXT_DIR:
    ...
    else 
    {
      ...
      GUI_SetTextColor(GUI_COLOR_WHITE);
      GUI_DisplayStringAt(40, LINE(10), (uint8_t*)"End of validation.", LEFT_MODE);
      GUI_DisplayStringAt(40, LINE(11), (uint8_t*)"Press wake-up", LEFT_MODE);
      GUI_DisplayStringAt(40, LINE(12), (uint8_t*)"button to see report", LEFT_MODE);
      DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
...
/**
* @brief Run test: function called before each PrepProcess stage for following purposes: timing measurement + Dump +  capture
* @param Test_Context_Ptr pointer to utilities context
* @param Operating_Mode curren toperating mode
* @retval None
*/
void TEST_Run(TestContext_TypeDef *TestContext_Ptr, AppOperatingMode_TypeDef Operating_Mode)
{
  ...
  if((Operating_Mode == DUMP) && (TestContext_Ptr->TestRunContext.src_buff_addr != NULL))
  {
    ...
    case 1:
      ...
      //BSP_LED_On(LED_RED);
      GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"DUMP BUSY ", RIGHT_MODE);
      ...
      {/**Dump intermediate data in SDCARD**/
        ...
        if(App_Cxt_Ptr->Test_ContextPtr->DumpContext.Dump_FrameSource != SDCARD_FILE)
        {
          ...
          if (BSP_SD_Init() != MSD_OK)
          {
            GUI_DisplayStringAt(0, LINE(12), (uint8_t *)"Error. SD Card not detected", CENTER_MODE);
            DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
            Error_Handler();
          }
        }
        ...
        {
          GUI_DisplayStringAt(0, LINE(12), (uint8_t *)"Error. Writting image failed", CENTER_MODE);
          DISPLAY_Refresh(App_Cxt_Ptr->Display_ContextPtr);
          Error_Handler();
        }      
      }
  ...
  else if((Operating_Mode == CAPTURE) && (TestContext_Ptr->TestRunContext.PerformCapture == 1) && (TestContext_Ptr->TestRunContext.src_buff_addr != NULL)) 
  {
    ...
    case 1:
      BSP_LED_Off(LED_GREEN);
      //BSP_LED_On(LED_RED);
      GUI_DisplayStringAt(0, LINE(1), (uint8_t *)"CAPTURE BUSY ", RIGHT_MODE);
