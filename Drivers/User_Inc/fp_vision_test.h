...
typedef enum
{
  ...
  LAUNCH_DUMP_CMD                  = 0x0A, /*Launch the DUMP mode by specifying: 1st) the DUMP sub-mode, i.e. the image frame source: SDCARD, CAMERA LIVE  or CAMERA TEST COLOR BAR, 2nd) the memory location where the intermediates data are dumped to: SDCARD (0x00) or SDRAM (0x01)*/
  TRIGGER_DUMP_CMD                 = 0x0B, /*Once in a specified DUMP sub-mode, triggers a given number (provided as param) of consecutive DUMP of the memory*/
  LAUNCH_CAPTURE_CMD               = 0x0C, /*Launch the CAPTURE mode: input param are: Format, inter-Capture delay in ms (if ==0 => 'manual' mode, else 'automatic mode), Number of captures to trigger (if in 'automatic' mode))*/
  TRIGGER_CAPTURE_CMD              = 0x0D, /*Once in CAPTURE mode, triggers a CAPTURE if 'manual' mode has been selected, otherwise it has no effect*/
  GET_VALIDATION_OUTPUT_SIZE_CMD   = 0x0E, /*Return the size of the data that will be uploaded upon reception of GET_VALIDATION_OUTPUT_CMD command*/
  UPLOAD_VALIDATION_OUTPUT_CMD     = 0x0F, /*Upload to the host the output of the Validation located in VALIDATION (?) folder on SDCARD: confusion matrix only (NN output for each single file/inference is not uploaded at that point)*/
  GET_DUMP_OUTPUT_DATA_SIZE_CMD    = 0x10, /*Return the size of the data that will be uploaded upon reception of UPLOAD_DUMP_OUTPUT_DATA_CMD command*/
  UPLOAD_DUMP_OUTPUT_DATA_CMD      = 0x11, /*Upload to the host the output data of the Dump (i.e. NN output) stored in the dump_output_buff buffer*/
  GET_DUMP_WHOLE_DATA_SIZE_CMD     = 0x12, /*Return the size of the data that will be uploaded upon reception of UPLOAD_DUMP_WHOLE_DATA_CMD command*/
  UPLOAD_DUMP_WHOLE_DATA_CMD       = 0x13, /*Upload to the host the whole data of the Dump (i.e. NN output buffer but also all the intermediate buffers) stored in the dump_intermediate_data_ping/pong_buff buffers*/
  READ_CAMERA_REGISTER_CMD         = 0x14, /*Reads the content of one camera register or a list of camera registers */
  WRITE_CAMERA_REGISTER_CMD        = 0x15, /*Writes the content of a camera register*/
  SET_CAMERA_MODE_CMD              = 0x16, /*Configure the camera in test bar or normal mode*/
  SET_CONFIG_SDCARD_PATH_CMD       = 0x17, /*Set the path (on SD card) where to write the results (validation + dump test bar) for a given config (i.e. binary)*/
